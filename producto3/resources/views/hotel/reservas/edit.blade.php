{{-- resources/views/hotel/reservas/edit.blade.php --}}
@php
    $minDate = now()->format('Y-m-d');
    $horaEntrada = $reserva->hora_entrada ? substr($reserva->hora_entrada, 0, 5) : null;
    $horaSalida  = $reserva->hora_vuelo_salida ? substr($reserva->hora_vuelo_salida, 0, 5) : null;
@endphp

<x-app-layout>
    <x-slot name="header">
        <h2 class="h4 mb-0">{{ __('Editar reserva (hotel)') }}</h2>
    </x-slot>
                                ...
                                <select name="id_viajero" id="id_viajero"
                                    ...
                                    @foreach ($viajeros as $viajero)
                                        <option value="{{ $viajero->id_viajero }}"
                                            @selected(old('id_viajero', $reserva->id_viajero) == $viajero->id_viajero)>
                                ...
                                <select name="id_tipo_reserva" id="id_tipo_reserva"
                                    ...
                                    @foreach ($tiposReserva as $tipo)
                                        <option value="{{ $tipo->id_tipo_reserva }}"
                                            @selected((string) old('id_tipo_reserva', $reserva->id_tipo_reserva) === (string) $tipo->id_tipo_reserva)>
                                            {{ $tipo->descripcion }}
                                        </option>
                                    @endforeach
                                </select>
                                ...
                                <div class="row">
                                    ...
                                    <div class="col-md-6 mb-3">
                                        ...
                                        <x-text-input id="hora_entrada" name="hora_entrada" type="time"
                                                      class="w-100"
                                                      :value="old('hora_entrada', $horaEntrada)" />
                                ...
                                <div class="row">
                                    ...
                                    <div class="col-md-6 mb-3">
                                        ...
                                        <x-text-input id="hora_vuelo_salida" name="hora_vuelo_salida" type="time"
                                                      class="w-100"
                                                      :value="old('hora_vuelo_salida', $horaSalida)" />
                                        ...
                                        <select id="id_vehiculo" name="id_vehiculo"
                                            ...
                                            @foreach ($vehiculos as $vehiculo)
                                                <option value="{{ $vehiculo->id_vehiculo }}"
                                                        ...
                                                        @selected((string) old('id_vehiculo', $reserva->id_vehiculo) === (string) $vehiculo->id_vehiculo)>
