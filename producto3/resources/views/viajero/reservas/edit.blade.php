                                    ...
                                    <div class="col-md-8 mb-4">
                                        ...
                                        <select id="id_vehiculo" name="id_vehiculo" class="form-select @error('id_vehiculo') is-invalid @enderror">
                                            ...
                                            @foreach ($vehiculos as $vehiculo)
                                                @php
                                                    // Filtramos los precios para este vehículo y sacamos los IDs de los hoteles
                                                    $hotelesPermitidos = $precios->where('id_vehiculo', $vehiculo->id_vehiculo)->pluck('id_hotel')->toArray();
                                                @endphp
                                                <option value="{{ $vehiculo->id_vehiculo }}"
                                                    data-plazas="{{ $vehiculo->plazas }}"
                                                    data-hoteles="{{ implode(',', $hotelesPermitidos) }}" {{-- <--- NUEVO --}}
                                                    {{ (string) $idVehiculoValue === (string) $vehiculo->id_vehiculo ? 'selected' : '' }}>
        ...
        <script>
            document.addEventListener('DOMContentLoaded', function () {
                ...
                function filtrarVehiculosPorPlazas() {
                    ...
                    const vehiculoActual = vehiculoSelect.value;
                    ...
                    Array.from(vehiculoSelect.options).forEach(option => {
                        ...
                        const tienePlazas = (n <= 0 || plazas >= n);
                    
                        // 2. Validar Hotel (Precio)
                        ...
                        if (tienePlazas && tienePrecio) {
                            option.disabled = false;
                            option.style.display = 'block'; // Para navegadores que lo soportan
                        } else {
                            option.disabled = true;
                            option.style.display = 'none';
                            
                            // Si el que estaba seleccionado ahora está deshabilitado, desmarcarlo
                            if (option.value === vehiculoActual) {
                ...
                if (hotelSelect) {
                    hotelSelect.addEventListener('change', filtrarVehiculosPorPlazas);
                }
