...
class ReservaController extends Controller
{
    ...
    public function create()
    {
        ...
        $viajero        = $user->viajero;
        $hoteles        = $hoteles = Hotel::whereHas('user', function($query) {
    ...
    {
        $user = Auth::user();
        abort_unless($user->isViajero(), 403, 'No tienes rol de viajero.');

        if (!$user->viajero || $reserva->id_viajero != $user->viajero->id_viajero) {
        ...
        return view('viajero.reservas.edit', compact('reserva', 'hoteles', 'vehiculos', 'tiposReserva', 'maxPlazas', 'precios'));
    }
    ...
    //public function update(Request $request, string $id)
    {
        ...
        $esIda    = in_array($tipo, [1, 3], true); // solo ida o ida+vuelta
        $esVuelta = in_array($tipo, [2, 3], true); // solo vuelta o ida+vuelta
        $tarifa = \App\Models\Precio::where('id_hotel', $data['id_hotel'])
                    ->where('id_vehiculo', $data['id_vehiculo'])
                    ->first();

        if (!$tarifa) {
            return back()->withErrors(['id_vehiculo' => 'Este vehículo no tiene una tarifa configurada para el hotel seleccionado.'])->withInput();
        }
        ...
        */
        ...
        |--------------------------------------------------------------------------
        */
        DB::transaction(function () use ($reserva, $viajero, $data, $tipo, $tarifa) {
            // Campos comunes
            $reserva->id_hotel_destino  = $data['id_hotel'];
            $reserva->id_hotel  = $data['id_hotel'];
    ...
    public function destroy(Reserva $reserva)
    {
        ...
        if ($user->isViajero()) {
            ...
            if ($reserva->fecha_entrada->isBefore(now()->addHours(48))) {
                return redirect()->route('viajero.reservas.index')
                    ->with('error', 'No puedes cancelar con menos de 48h de antelación.');
            }
        }
